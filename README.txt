...
直接说「笑一个」「害羞一点」「跳个舞」，她会自己挑**最接近**的表情 / 动作切过去。

| 工具 | 作用 |
|:---|:---|
| `set_live2d_expression` | 切换表情（「笑一个」→ `开心兴奋` / `星星眼`） |
| `play_live2d_motion` | 播一次动作（「跳个舞」→ `自拍` / `喷水`），播完自己停 |
| `clear_live2d_expression` | 回到模型默认脸（「别笑了」） |

- 可用名字由 `core/live2d_assets` 扫描模型目录**实时得出并写进工具说明**，
  所以 AI 一次调用就能挑到最接近的；名字对不上时返回**完整清单**让它自我纠正，
  而不是静默失败
- AI 设的表情会**保持到本轮回复结束**，之后自动恢复成由对话状态驱动的表情。
  没有这个保持期的话，工具一返回、对话阶段变成 `writing`，表情立刻被换成
  `state_map` 里的「画笔」，等于白设
- 动作是一次性的（播完自己停），与状态机的动作不冲突，不受保持期影响

> ⚠️ 实现上有个绕不开的约束：**工具跑在 worker 线程，而 Live2D 是 Qt 控件**，
> 跨线程调用会崩。所以插件只往 `core/live2d_control` 的队列里塞一条请求，
> 由 UI 的定时器搬到 GUI 线程执行 —— 与 `core/brain.py` 上报对话阶段是同一种做法
> （core 存状态、UI 来读），不引入 core → Qt 的反向依赖。
